...
- **Local HTTPS note:** since this runs locally for now, a real device needs either `adb reverse tcp:4000 tcp:4000` (USB) or a TLS tunnel (e.g. ngrok) to reach it. Plain `http://` to a non-localhost host requires a network-security-config cleartext exception on Android — keep that dev-only and never ship it in a release build.
