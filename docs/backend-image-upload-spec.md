...
| `GET` | `/users/me` | Required | — | `200` camelCase profile JSON (`firstName`, `lastName`, `email`, `photoUrl`, `photoSource`, `dateOfBirth`, `gender`, `phoneRegionCode`, `phoneDialCode`, `phoneNumber`). `404` if the caller has no profile row yet. |
| `PUT` | `/users/me` | Required | JSON body matching that camelCase shape | `200` the saved profile |

`:context` is whitelisted server-side. Today only `profile-picture` is valid; `hazard-report` is reserved for later and should get its own folder, size/dimension limits, and an additional ownership check (e.g. a `reportId` field validated against the backend's own report store) once that feature exists. Reject any other value with `400`.

`GET`/`PUT` `/users/me` are the profile store the Android app uses (`UserApi`). The backend must resolve the row from the verified Supabase `uid`, not from a path parameter.
...
3. Supabase JWT verification (see §2). Reject with `401` if missing/invalid.
...
- Supabase JWT verification credentials (`SUPABASE_URL` / JWT secret or JWKS) are environment-only too (`.env`, gitignored; commit a `.env.example` with empty placeholders). Never put the service-role key in the Android app.
...
| `SUPABASE_URL` | Used to fetch JWKS / identify the project when verifying access tokens |
| `SUPABASE_JWT_SECRET` | Server-only JWT secret (or equivalent JWKS setup) for verifying access tokens |
...
- Every request automatically carries `Authorization: Bearer <Supabase access token>` via `NetworkModule`'s OkHttp interceptor — the app never attaches it manually.
- On a successful profile-picture upload, the app immediately persists `photoUrl = secureUrl` and `photoSource = CLOUDINARY` with `PUT /users/me` (independent of any other pending profile edits).
